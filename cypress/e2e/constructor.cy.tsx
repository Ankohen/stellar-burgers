describe('Конструктор бургера', () => {
  ...
  beforeEach(() => {
    ...
    // Алиасы для кнопок
    cy.get(`${selectors.ingredients.bun}`)
      .first()
      .find('button')
      .as('bunAddButton');
    cy.get(`${selectors.ingredients.sauce}`)
      .last()
      .find('button')
      .as('sauceAddButton');
    cy.get(`${selectors.ingredients.main}`)
      .eq(1)
      .find('button')
      .as('mainAddButton');
    cy.get(`${selectors.ingredients.bun}`)
  ...
  context('Добавление булки и начинки в конструктор', () => {
    it('добавить булочку и соус в конструктор', () => {
      ...
      cy.get(selectors.constructor.sauce)
        .should('exist')
